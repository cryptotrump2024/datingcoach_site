...
const values = [
  ...
  { icon: <Brain className="w-8 h-8" />, title: 'Science-Backed Methods', desc: 'Every technique we teach is grounded in peer-reviewed psychology research on communication, attraction, and relationship dynamics.', color: '#8B5CF6' },
  { icon: <Shield className="w-8 h-8" />, title: 'Privacy First', desc: 'Your conversations and data are encrypted and never sold. What you practice here stays here. Period.', color: '#10B981' },
]
    ...
    <div className="min-h-[100dvh]" style={{ background: '#0A0A0F' }}>
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="relative max-w-[800px] mx-auto px-6 text-center">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: easeOutExpo }} className="text-caption uppercase tracking-[0.15em] block mb-4" style={{ color: '#52525B' }}>Our Story</motion.span>
      ...
      <section ref={ref1} className="relative py-32" style={{ background: '#0A0A0F' }}>
        <div className="max-w-[800px] mx-auto px-6">
          {chapters.map((c, i) => (
            <motion.div key={c.label} initial={{ opacity: 0, y: 50 }} animate={iv1 ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.15, ease: easeOutExpo }} className="mb-16 last:mb-0">
              ...
              <p className="text-body-lg leading-[1.75]" style={{ color: '#A1A1AA' }}>{c.text}</p>
            </motion.div>
          ))}
        </div>
      </section>

      <section ref={ref2} className="relative py-24" style={{ background: '#0E0E15' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((s, i) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 30 }} animate={iv2 ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: i * 0.1, ease: easeOutExpo }} className="text-center glass-card p-6">
                <div className="flex justify-center mb-3" style={{ color: '#FB7185' }}>{s.icon}</div>
                <div className="text-heading-xl font-bold text-[#F5F5F7]">{s.value}</div>
                <div className="text-caption mt-1" style={{ color: '#52525B' }}>{s.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section ref={ref3} className="relative py-32" style={{ background: '#0A0A0F' }}>
        <div className="max-w-[1000px] mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={iv3 ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, ease: easeOutExpo }} className="text-center mb-16">
            ...
            <p className="text-body-lg" style={{ color: '#A1A1AA' }}>Anonymous by design. Our work speaks for itself.</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {team.map((m, i) => (
              <motion.div key={m.role} initial={{ opacity: 0, y: 40 }} animate={iv3 ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.15 * i, ease: easeOutExpo }} className="glass-card p-8 text-center">
                ...
                <h3 className="text-heading-md text-[#F5F5F7] mb-3">{m.role}</h3>
                <p className="text-body-sm" style={{ color: '#A1A1AA' }}>{m.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section ref={ref4} className="relative py-32" style={{ background: '#0E0E15' }}>
        <div className="max-w-[1000px] mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {values.map((v, i) => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 40 }} animate={iv4 ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.12 * i, ease: easeOutExpo }} className="glass-card p-8" style={{ borderLeft: `3px solid ${v.color}` }}>
                ...
                <h3 className="text-heading-md text-[#F5F5F7] mb-3">{v.title}</h3>
                <p className="text-body-sm leading-relaxed" style={{ color: '#A1A1AA' }}>{v.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-32" style={{ background: '#0A0A0F' }}>
        ...
        <div className="relative max-w-[600px] mx-auto px-6 text-center">
          <h2 className="text-display-subsection text-[#F5F5F7] mb-4">Ready to Start Your Journey?</h2>
          <p className="text-body-lg mb-8" style={{ color: '#A1A1AA' }}>Every expert was once a beginner. Your first conversation practice is free.</p>
          <Link to="/create" className="inline-flex items-center gap-2 btn-gradient text-[#F5F5F7] text-body-lg font-semibold px-8 py-4 rounded-full shadow-glow-rose hover:scale-105 transition-transform">
